...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Sanitize API key model policies.
...
// SanitizeAPIKeyPolicies normalizes and deduplicates per-key model policies.
func (cfg *Config) SanitizeAPIKeyPolicies() {
	if cfg == nil || len(cfg.APIKeyPolicies) == 0 {
		return
	}

	seenKeys := make(map[string]struct{}, len(cfg.APIKeyPolicies))
